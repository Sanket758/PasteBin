#Project By> Frankiii
#Source Code> Frankiii
#API> https://pastebin.com
# Questions > sanketgadge007@gmail.com
...
def main(argv):
	...
	try:
		# A continuous loop until user stops it using ctrl+c 
		while True:

			# Get pastebin.com Homepage
			root_html = BeautifulSoup(fetch_page(root_url), 'html.parser')
			
			#	selecting each paste one by one
			for paste in find_new_pastes(root_html):
				
				#	check length of paste_list before adding a new element
				...
				if len(paste_list) > length:
					...
					#	Add the url to found_keywords if it contains keywords
			...
			# Exit if max time limit reached 
